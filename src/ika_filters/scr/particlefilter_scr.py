...
class ParticleFilter:
    ...
    def init_particles(self, seedHeading: float = 0.0, useSeedHeading: bool = False):
        ...
        if useSeedHeading:
            self.particles = [
                Particle(0, 0, seedHeading + np.random.normal(0, 0.1))
                for i in range(self.num_particles)
            ]
        else:
            self.particles = [
                Particle(0, 0, i / self.num_particles * 2 * math.pi)
                for i in range(self.num_particles)
            ]
    ...
    def feedback(self, feedback: MotorFeedback) -> list[float]:
        ...
        for particle in self.particles:
            particle.x += feedback.delta_x * 0.95 * math.cos(
                particle.theta
            ) + feedback.delta_y * math.sin(particle.theta)
            particle.y += feedback.delta_x * 0.95 * math.sin(
                particle.theta
        ...
        avg_theta = math.atan2(sum_theta_y / sum_weight, sum_theta_x / sum_weight) % (
            2 * math.pi
        )
    ...
    def resample(self) -> None:
        ...
        for particle in new_particles:
            ...
            x = (
                particle.x
                + rand_x * math.cos(particle.theta)
                + rand_y * math.sin(particle.theta)
            )
            y = (
                particle.y
                + rand_x * math.sin(particle.theta)
