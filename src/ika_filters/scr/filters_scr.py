...
class FiltersNode(Node):
    def __init__(self):
        ...
        self.latitude_length = (
            self.declare_parameter("latitude_length", 111086.2)
            .get_parameter_value()
            .double_value
        )
        self.longitude_length = (
            self.declare_parameter("longitude_length", 81978.2)
            .get_parameter_value()
            .double_value
        )
    ...
    def config_updated(self, jsonObject):
        self.config = json.loads(
            self.jdump(jsonObject), object_hook=lambda d: SimpleNamespace(**d)
        )
        self.onReset()
    ...
    def init(self):
        ...
        self.create_subscription(
            MotorFeedback, "/autonav/MotorFeedback", self.onMotorFeedbackReceived, 20
        )
        self.position_publisher = self.create_publisher(
            Position, "/autonav/position", 20
        )
    ...
    def system_state_transition(self, old: SystemState, updated: SystemState):
        if (
            old.state != SystemStateEnum.AUTONOMOUS
            and updated.state == SystemStateEnum.AUTONOMOUS
        ):
            self.onReset()

        if (
            old.state != SystemStateEnum.MANUAL
            and updated.state == SystemStateEnum.MANUAL
        ):
            self.onReset()
