...
# front_udev = "/dev/v4l/by-id/usb-046d_C270_HD_WEBCAM_200901010001-video"
...
class CameraNode(Node):
    def __init__(self):
        ...
        self.config.output_width = self.declare_parameter("width", 1920).value
        self.config.output_height = self.declare_parameter("height", 1080).value
        self.camera_path = self.declare_parameter(
            "udev_path",
            "/dev/v4l/by-id/usb-046d_C922_Pro_Stream_Webcam_944FA6DF-video-index0",
        ).value
        self.topic_name = self.declare_parameter(
            "topic_name", "/shoot_camera/image_raw"
        ).value

        self.camera_publisher = self.create_publisher(Image, self.topic_name, 10)
