...
class JoyController(Node):
    def __init__(self):
        ...
        self.mode_pub = self.create_publisher(
            String, "/ika_controller/mode", qos_profile
        )
        self.cmd_pub = self.create_publisher(
            String, "/ika_controller/joy_cmd", qos_profile
        )
    ...
    def publish_loop(self):
        ...
        if self.mode == "A":
            self.compute_wheel_velocities(
                self.auto_vel.linear.x, self.auto_vel.angular.z
            )
        else:
            self.compute_wheel_velocities(self.lin_vel, self.ang_vel)
    ...
    def compute_wheel_velocities(self, lin_vel, ang_vel):
        v = np.array([[lin_vel], [ang_vel]])
    ...
    def build_command_string(self):
        ...
        command_str = f"{self.mode}{self.right_direct}{abs((self.wheel_vel_right)):.2f}{self.left_direct}{(abs(self.wheel_vel_left)):.2f}{tilt_cmd}{pan_cmd}{laser_on}{reset_card}"
        if self.lin_vel < 0:
            command_str = f"{self.mode}{self.right_direct}{abs((self.wheel_vel_left)):.2f}{self.left_direct}{abs((self.wheel_vel_right)):.2f}{tilt_cmd}{pan_cmd}{laser_on}{reset_card}"
        return command_str
