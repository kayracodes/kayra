...
class CameraShooter(Node):
    def __init__(self):
        ...
        self.create_subscription(
            RoadSignArray, "/road_signs", self.road_signs_callback, 10
        )
    ...
    def road_signs_callback(self, msg: RoadSignArray):
        ...
        if len(self.road_signs.signs) > 0:
            self.last_sign = self.sign_id_map[self.road_signs.signs[0].id]
        ...
        if self.last_sign == "12" and not self.is_target_hit:
            ...
            if not self.tilt_complete:
                ...
                if abs(pan) > self.pan_sensitivity:
                    self.pan_on = True
                    self.pan_dir = "0" if pan < 0.0 else "1"
                    self.laser_on = False
                else:
            ...
            self.get_logger().info(
                f"Sending auto cmd: {self.auto_vel.v:.2f}, {self.auto_vel.omega:.2f}"
            )
        self.command_str = self.generate_serial_cmd()
    ...
    def compute_wheel_velocities(self):
        v = np.array([[self.auto_vel.v], [self.auto_vel.omega]])
    ...
    def generate_serial_cmd(self):
        ...
        return f"{mode}{cmd_str}{tilt_cmd}{pan_cmd}{laser_on}CF"
